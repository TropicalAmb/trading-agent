...
- 2026-08-14: **Windows recovery tasks restored and verified** — a healthy directly launched supervisor was running without the two independent Task Scheduler recovery entries. Reinstalled `TradingAgentWatchdog` (Hidden, `WakeToRun=false`, `IgnoreNew`, every 5m) and `TradingAgentAutonomous` (Hidden, `WakeToRun=false`, `IgnoreNew`, at login) using the existing silent `pythonw.exe` actions. The watchdog's first automatic run completed at 10:56:03 ET with result `0`; heartbeat and supervisor remained healthy. Runtime/strategy/risk/quantity/universe/max positions/1m/paper mode unchanged.
