...
**Research data (binding):** Prefer **Databento volume-continuous + Yahoo dual**. On 2026-08-14 the user approved and the system spent an estimated **$1.285970583558** to download 180d of NQ.v.0 (177,188 rows) and CL.v.0 (175,049 rows). Legacy parent caches are preserved under `data/databento/legacy_parent_20260814_autonomy4/`, never used as single-series evidence. NQ audit PASS; CL WARN (0.6021% 2–30m gaps); both have zero duplicate timestamps/invalid OHLC and strong Yahoo 5m return agreement (NQ 0.9843, CL 0.9490). Six source dates are Databento-degraded. A quote-first TBBO pipeline is now prepared for genuine aggressor-side trades plus pre-trade top-of-book; its prior free metadata quote for 30d NQ.v.0+CL.v.0 was **$23.170063**, but no TBBO was downloaded and it has produced no strategy result. **No further Databento download without fresh user OK.**
...
- 2026-08-14: **Guarded Databento TBBO pipeline prepared — zero download/spend, no profitability claim** — stopped further cosmetic OHLCV-indicator expansion. Added strict decoding/aggregation for genuine TBBO trade-side and pre-trade BBO fields: Databento `B`=buyer aggressor, `A`=seller aggressor; one/five-minute output includes buy/sell/unknown volume, signed delta, delta ratio, trade VWAP, spread, and top-of-book size imbalance. Missing schema fields, unknown side encodings, non-trades, invalid prices/sizes, unsorted event time, and locked/crossed quotes fail closed. Added `scripts/cache_databento_tbbo.py`, which is quote-only by default and cannot make a paid request without `--download`, exact `--confirm-spend USER_APPROVED`, a positive cap, and a quote at/below that cap; request code is fixed to 30d maximum GLBX.MDP3/TBBO/continuous NQ.v.0+CL.v.0, raw files are ignored, and API keys are never printed. The already-recorded free 30d NQ+CL quote is $23.170063. Full regression is 264 passed. This turn made no Databento API call, downloaded nothing, spent $0, promoted no strategy, and changed no paper/config/risk/quantity/universe/session/poll/watchdog behavior.
